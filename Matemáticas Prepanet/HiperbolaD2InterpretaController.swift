//
//  HiperbolaD2InterpretaController.swift
...
class HiperbolaD2InterpretaController: UIView {
    ...
    override init(frame: CGRect) {
        ...
        xnumber = Int.random(in: 0 ... 100)
        y1number = Int.random(in: 0 ... 100)
        y2number = Int.random(in: 0 ... 100)
        bnumber = Int.random(in: 0 ... 100)
         while(y1number < y2number){
            y1number = Int.random(in: 0 ... 100)
            y2number = Int.random(in: 0 ... 100)
        }
        while(abs(y1number - y2number) < bnumber){
    ...
    func commonInit(){
        ...
        Bundle.main.loadNibNamed("HiperbolaD2Interpreta", owner: self, options: nil)
        
        lbVX?.text = String(xnumber)
        lbVY?.text = String(y1number)
        lbCX?.text = String(xnumber)
        lbCY?.text = String(y2number)
        lbA?.text = String(abs(y1number - y2number))
        lbB?.text = String(bnumber)
